...
# Sanity check: duplicate (name, country) keys within a single CF year would
# silently collapse into one aggregated row and get attributed to any IOI
# person sharing that key.
cf_year_dupes <- cf %>% count(name_key, country_key, year) %>% filter(n > 1)
cat(sprintf("[diag] CF rows with duplicate (name,country,year): %d\n",
            nrow(cf_year_dupes)))

ioi_dupes <- ioi %>% count(name_key, country_key) %>% filter(n > 1)
cat(sprintf("[diag] IOI rows with duplicate (name,country):     %d\n",
            nrow(ioi_dupes)))
...
# The headline rate above includes IOI participants from before CF existed
# (CF coverage starts 2011). Condition on lastYear >= 2011 for a fair rate.
