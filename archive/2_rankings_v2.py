...
def calculate_ioi_timing_statistics(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate statistics about timing between CF registration and IOI participation
    
    Args:
        df: DataFrame with Year and CF_Registration_Year columns
        
    Returns:
        DataFrame with additional timing statistics
    """
    ...
    # For each unique handle, find their first IOI year
    df[first_ioi_col] = df.groupby(handle_col)[year_col].transform('min')
    
    # Calculate years before first IOI (negative means registered after IOI)
...
def main():
    ...
    print(f"\n⚠️  IMPORTANT CHANGE:")
    print(f"  Ratings are extracted 1 MONTH BEFORE each IOI competition")
    print(f"  This reflects participants' skill at competition time, not year-end")
    print(f"\nColumns created:")
