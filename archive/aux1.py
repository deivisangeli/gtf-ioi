...
class CPHOFScraper:
    """Scraper for CPHOF site - Using proven logic from working code"""
    ...
    def get_page_content(self, url: str):
        ...
        try:
            ...
            return BeautifulSoup(response.content, 'html.parser')  # Using .content like working code
        except Exception as e:
            logger.error(f"Error fetching {url}: {e}")
            return None
    
    def get_medal_info(self, rank_cell) -> tuple:
        """Extract medal information from rank cell - from working code"""
    ...
    def scrape_year(self, year: int) -> List[Dict]:
        """Scrape participants from cphof.org - using logic from working code"""
        ...
        # Look for the Final standings table - from working code
        ...
        # Find the table after the Final section
        table = None
        if final_section:
            table = final_section.find_next('table')
        
        # If no table found, look for any table with typical IOI columns
        if not table:
        ...
        for row_idx, row in enumerate(data_rows):
            ...
            for cell_idx, cell in enumerate(cells):
                ...
                # Second cell is usually country
                elif cell_idx == 1:
                    img = cell.find('img')
                ...
                # If no profile link, check if this looks like a name column
                if not participant_name and cell_text and not cell_text.isdigit():
                    if cell_idx != 1:  # Not country cell
                ...
                for link in links:
                    href = link.get('href', '')
                    if 'codeforces.com' in href.lower():
                        codeforces_url = href
                        break
            
            # If we found a participant, add them
            if participant_name:
                participants.append({
                    'Year': year,
                    ...
                    'Result': medal_type,
                    'CF_Link': codeforces_url if codeforces_url else None,
                    'Source': 'CPHOF'
                })
