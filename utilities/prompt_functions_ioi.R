...
###Function with prompts for the IOI participants
...
prompt_fun <- function(name, country, lastYear) {
  paste0(
    name, " has participated in the International Olimpyad in Informatics for the last time in ", lastYear, ", representing ", country, ".\n",
...
prompt_fun_Deivis <- function(name, country, lastYear) {
  paste0(
    name, " has participated in the International Olympiad in Informatics (IOI) for the last time in ", lastYear, ", representing ", country, ".\n",
    "You need to collect data on their career trajectory, for an academic study. Start by searching LinkedIn and other personal or institutional pages. Do NOT stop at the first profile you find or at the first failure; examine multiple sources and cross-check identities before concluding. E.g., ask if the last year of IOI participation makes sense with their current career stage, year of BA, etc. Read resumes or linkedin pages when available.\n",
    "If the person is from a country where English is not commonly spoken, also search in the local language in addition to English.\n\n",
    
    "When determining `BA_institution`, APPLY this pre-Bologna heuristic:\n",
    "- If the person is European AND the year is < 1999 AND the earliest post-secondary degree found is labeled as a master's/integrated long-cycle (e.g., 'Diplom', 'Magister', 'Laurea' pre-Bologna, 'Licenciatura' pre-Bologna, 'Dipl.-Ing.', or integrated master's such as 'MEng', 'MPhys'), AND that program starts within 0–2 years of the IOI participation (or at typical undergrad entry ages 17–21), THEN TREAT THAT DEGREE’S INSTITUTION AS the `BA_institution` (equivalent to a bachelor under the later three-cycle system).\n",
    "- If a clear standalone bachelor's exists, prefer it over the heuristic above.\n",
    "- If evidence is ambiguous, choose the most plausible first-cycle equivalent based on country timelines and degree structure, but still return ONLY the JSON below.\n\n",
    ...
    "- `recognitions`: List major internationally recognized prizes and fellowships given for career achievement. Do not include prizes that are very local, or that have unclear prestige. Also ignore student/doctoral grants and fellowships, and any IOI medals.\n",
...
prompt_fun_new_alt <- function(name, country, lastYear) {
  paste0(
    name, " has participated in the International Olimpyad in Informatics for the last time ", lastYear, ", representing ", country, ".\n",
    "You need to collect data on their career trajectory, for an academic study. Start by searching LinkedIn and other personal or institutional pages. Do NOT stop at the first profile you find or at the first failure; examine multiple sources and cross-check identities before concluding. E.g., ask if the last year of IMO participation makes sense with their current career stage, year of BA, etc. Read resumes when available.\n",
    "If the person is from a country where English is not commonly spoken, also search in the local language in addition to English.\n\n",
...
prompt_fun_linkedin <- function(name, country, lastYear) {
  paste0(
    name, " participated for the last time in the International Olimpyad in Informatics in ", lastYear,
